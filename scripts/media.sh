...
function media-to-remote {
    ...
    ${RSYNC} -a --no-o --no-g -z -e "ssh -p ${SSH_PORT}" --stats ${LOCAL_ROOTDIR}${MEDIA_DIR}* ${REMOTE_SSH_USER}@${REMOTE_SSH_HOST}:${REMOTE_ROOTDIR}${MEDIA_DIR}
    ${RSYNC} -a --no-o --no-g -z -e "ssh -p ${SSH_PORT}" --stats ${LOCAL_ROOTDIR}${MEDIA_DIR}.* ${REMOTE_SSH_USER}@${REMOTE_SSH_HOST}:${REMOTE_ROOTDIR}${MEDIA_DIR}
...
function media-to-local {
    ${RSYNC} -a --no-o --no-g -z -e "ssh -p ${SSH_PORT}"  --stats ${REMOTE_SSH_USER}@${REMOTE_SSH_HOST}:${REMOTE_ROOTDIR}${MEDIA_DIR}* ${LOCAL_ROOTDIR}${MEDIA_DIR}
    ${RSYNC} -a --no-o --no-g -z -e "ssh -p ${SSH_PORT}"  --stats ${REMOTE_SSH_USER}@${REMOTE_SSH_HOST}:${REMOTE_ROOTDIR}${MEDIA_DIR}.* ${LOCAL_ROOTDIR}${MEDIA_DIR}
