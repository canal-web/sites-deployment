...
function backup-remote {
    ...
    ssh -p ${SSH_PORT} ${REMOTE_SSH_USER}@${REMOTE_SSH_HOST} ${DUMP_COMMAND}

    # Copy it locally
    scp -P ${SSH_PORT} ${REMOTE_SSH_USER}@${REMOTE_SSH_HOST}:${DBREMOTE_DUMP_FILENAME} ${BACKUP_FOLDER}

    # Remove distant .sql file
    RM_COMMAND="rm -f ${DBREMOTE_DUMP_FILENAME}"
    ssh -p ${SSH_PORT} ${REMOTE_SSH_USER}@${REMOTE_SSH_HOST} ${RM_COMMAND}
